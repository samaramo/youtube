...
function(d) {
    ...
    b.getAbsolutePath = function(a, e) {
        ...
        if (b.exists(a)) {
            ...
            0 === a.indexOf("/") ? k = a.split("/") : (k = e.split("?")[0],
            k = k.substring(c.length + g.length + 1, k.lastIndexOf("/")),
            k = k.split("/").concat(a.split("/")));
    ...
    ;
    b.repo = function() {
        var a = "/" + d.version.split(/\W/).splice(0, 2).join("/") + "/";
        try {
    ...
    d.scriptloader = function(g) {
        ...
        this.load = function() {
            ...
            if (l && (l.getStatus() == a.NEW || l.getStatus() == a.LOADING))
                l.addEventListener(k.ERROR, c),
                l.addEventListener(k.COMPLETE, b);
            else if (d.scriptloader.loaders[g] = this,
            e == a.NEW) {
                e = a.LOADING;
                var n = h.createElement("script");
                n.addEventListener ? (n.onload = b,
                n.onerror = c) : n.readyState && (n.onreadystatechange = function() {
...
function(d) {
    ...
    d.plugin = function(g) {
        ...
        this.registerPlugin = function(b, e, c, d) {
            ...
            n = e;
            c && d ? (l = d,
            m = c) : "string" == typeof c ? l = c : "function" == typeof c ? m = c : !c && !d && (l = b);
            k = a.loaderstatus.COMPLETE;
            j.sendEvent(h.COMPLETE)
        }
        ;
        ...
        this.setupPlugins = function(b, e, c) {
            ...
            for (l in e.plugins) {
                var j = a.getPluginName(l)
                  ...
                  , n = m.getJS()
                  , q = m.getURL();
                B && (f.plugins[B] = a.extend({}, e.plugins[l]),
                f.plugins[B].pluginmode = m.getPluginmode(),
                f.length++);
                try {
      ...
      , c = d.embed = function(b) {
        ...
        s.addEventListener(h.COMPLETE, function() {
            if ("array" == a.typeOf(m.playlist) && 2 > m.playlist.length && (0 == m.playlist.length || !m.playlist[0].sources || 0 == m.playlist[0].sources.length))
                ...
                m.fallback ? (a.log("No suitable players found and fallback enabled"),
                new c.download(n,m,k)) : (a.log("No suitable players found and fallback disabled"),
                n.parentNode.replaceChild(j, n))
            }
        });
...
function(d) {
    ...
    (d.embed.config = function(b) {
        ...
        b.listbar && (b.playlistsize = b.listbar.size,
        ...
        if (r && r.length) {
            ...
            for (r = 0; r < s.length; r++) {
                ...
                if (u.file)
                    for (r in l)
                        t == l[r] ? (m = u.file,
                        j = p.image) : a.isYouTube(u.file) && (f = u.file)
            }
            m ? (b = m,
            d && (m = k("a", "display", d),
            k("div", "icon", m),
            k("div", "logo", m),
            b && m.setAttribute("href", a.getAbsolutePath(b))),
            b = "#" + d.id + " .jwdownload",
            d.style.width = "",
            d.style.height = "",
            e(b + "display", {
            ...
            d = k("embed", "", d),
            d.src = "http://www.youtube.com/v/" + /v[=\/](\w*)|\/(\w+)$|^(\w+)$/i.exec(c).slice(1).join(""),
    ...
    (d.embed.flash = function(c, b, e, k, l) {
        ...
        this.embed = function() {
            ...
            if (10 > f)
                ...
                !1;
            var d, p = a.extend({}, e);
            c.id + "_wrapper" == c.parentNode.id ? document.getElementById(c.id + "_wrapper") : (d = document.createElement("div"),
            d.id = c.id + "_wrapper",
            d.style.position = "relative",
            d.style.width = a.styleDimension(p.width),
            d.style.height = a.styleDimension(p.height),
            ...
            p.base = t + "/";
            g[c.id] = p;
            a.isIE() ? (p = '\x3cobject classid\x3d"clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" " width\x3d"100%" height\x3d"100%" id\x3d"' + c.id + '" name\x3d"' + c.id + '" tabindex\x3d0""\x3e',
            p += '\x3cparam name\x3d"movie" value\x3d"' + b.src + '"\x3e',
            p += '\x3cparam name\x3d"allowfullscreen" value\x3d"true"\x3e\x3cparam name\x3d"allowscriptaccess" value\x3d"always"\x3e',
            p += '\x3cparam name\x3d"seamlesstabbing" value\x3d"true"\x3e',
            p += '\x3cparam name\x3d"wmode" value\x3d"' + d + '"\x3e',
            p += '\x3cparam name\x3d"bgcolor" value\x3d"#000000"\x3e',
            p += "\x3c/object\x3e",
            c.outerHTML = p,
            p = document.getElementById(c.id)) : (p = document.createElement("object"),
            p.setAttribute("type", "application/x-shockwave-flash"),
            p.setAttribute("data", b.src),
            p.setAttribute("width", "100%"),
            p.setAttribute("height", "100%"),
            p.setAttribute("bgcolor", "#000000"),
            p.setAttribute("id", c.id),
            p.setAttribute("name", c.id),
            p.setAttribute("tabindex", 0),
            m(p, "allowfullscreen", "true"),
            m(p, "allowscriptaccess", "always"),
            m(p, "seamlesstabbing", "true"),
            m(p, "wmode", d),
...
function(d) {
    var a = d.embed
      ...
      , g = h.extend(function(c) {
        ...
        delete g.related;
        if ("premium" == f || "ads" == f)
            e.sharing && (e.sharing.client && e.sharing.client.match(".js$|.swf$") && (n = e.sharing.client),
            g[n] = e.sharing),
            e.related && (e.related.client && e.related.client.match(".js$|.swf$") && (q = e.related.client),
            g[q] = e.related),
            e.ga && (e.ga.client && e.ga.client.match(".js$|.swf$") && (j = e.ga.client),
            g[j] = e.ga),
...
function(d) {
    var a = []
      ...
      , e = d.api = function(a) {
        ...
        function m(a, b) {
            p[a] || (p[a] = [],
            q(g.JWPLAYER_PLAYER_STATE, function(b) {
                var c = b.newstate;
                b = b.oldstate;
                if (c == a) {
                    var e = p[c];
        ...
        f.resize = function(a, c) {
            if ("flash" != f.renderingMode)
                ...
                e && (e.style.width = h.styleDimension(a),
                e.style.height = h.styleDimension(c))
            }
            return f
        }
        ;
        f.play = function(a) {
            "undefined" == typeof a ? (a = f.getState(),
            a == c.PLAYING || a == c.BUFFERING ? j("jwPause") : j("jwPlay")) : j("jwPlay", a);
            return f
        }
        ;
        f.pause = function(a) {
            "undefined" == typeof a ? (a = f.getState(),
            a == c.PLAYING || a == c.BUFFERING ? j("jwPause") : j("jwPlay")) : j("jwPause", a);
            return f
        }
        ;
        f.stop = function() {
    ...
    e.destroyPlayer = function(c) {
        for (var e = -1, d, g = 0; g < a.length; g++)
            ...
            d = a[g]);
        0 <= e && (c = d.id,
        g = b.getElementById(c + ("flash" == d.renderingMode ? "_wrapper" : "")),
        h.clearCss && h.clearCss("#" + c),
        g && ("html5" == d.renderingMode && d.destroyPlayer(),
